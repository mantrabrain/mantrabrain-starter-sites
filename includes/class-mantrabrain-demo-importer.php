...
/*
	...
	private function get_demo_packages() {
		$packages = get_transient( 'mantrabrain_starter_sites_packages' );
		$template = strtolower( str_replace( '-pro', '', get_option( 'template' ) ) );

		if ( false === $packages || ( isset( $packages->slug ) && $template !== $packages->slug ) ) {
			$raw_packages = wp_safe_remote_get( "https://raw.githubusercontent.com/mantrabrain/starter-site-package/master/configs/{$template}.json" );
            //$raw_packages = wp_safe_remote_get ( "http://localhost/WordpressThemes/mantra-news/wp-content/starter-site-package/configs/{$template}.json" );
			if ( ! is_wp_error( $raw_packages ) ) {
				$packages = json_decode( wp_remote_retrieve_body( $raw_packages ) );

				if ( $packages ) {
					set_transient( 'mantrabrain_starter_sites_packages', $packages, WEEK_IN_SECONDS );
				}
			}
		}

		return apply_filters( 'mantrabrain_starter_sites_packages_' . $template, $packages );
	}
	...
	public function ajax_query_demos( $return = true ) {
		...
		if ( isset( $available_packages->demos ) ) {
			foreach ( $available_packages->demos as $package_slug => $package_data ) {
				...
				$screenshot_url = "https://raw.githubusercontent.com/mantrabrain/starter-site-package/master/resources/{$available_packages->slug}/{$package_slug}/screenshot.png";
	...
	public function ajax_import_demo() {
		...
		include_once( dirname( __FILE__ ) . '/admin/class-mantrabrain-demo-pack-upgrader.php' );

		$skin     = new WP_Ajax_Upgrader_Skin();
		$upgrader = new Mantrabrain_Demo_Pack_Upgrader( $skin );
		$template = strtolower( str_replace( '-pro', '', get_option( 'template' ) ) );
		$packages = isset( $this->demo_packages->demos ) ? json_decode( wp_json_encode( $this->demo_packages->demos ), true ) : array();
		$zip_uri = "https://github.com/mantrabrain/starter-site-package/raw/master/packages/{$template}/{$slug}.zip";
		//$zip_uri = "http://localhost/WordpressThemes/mantra-news/wp-content/starter-site-package/packages/{$template}/{$slug}.zip";
		///echo $zip_uri;exit;
		$result   = $upgrader->install( $zip_uri );
	...
	public function import_dummy_xml( $demo_id, $demo_data, $status ) {
		...
		if ( is_file( $import_file ) ) {
			...
			$status['errorMessage'] = __( 'The XML file dummy content is missing.', 'mantrabrain-starter-sites' );
			wp_send_json_error( $status );
		}
	...
	public function import_customizer_data( $demo_id, $demo_data, $status ) {
		...
		if ( is_file( $import_file ) ) {
			...
			$status['errorMessage'] = __( 'The DAT file customizer data is missing.', 'mantrabrain-starter-sites' );
			wp_send_json_error( $status );
		}
	...
	public function import_widget_settings( $demo_id, $demo_data, $status ) {
		...
		if ( is_file( $import_file ) ) {
			...
			$status['errorMessage'] = __( 'The WIE file widget content is missing.', 'mantrabrain-starter-sites' );
			wp_send_json_error( $status );
		}
