...
/**
 * Mantrabrain_Customizer_Importer Class.
 */
class Mantrabrain_Customizer_Importer {
	...
	public static function import( $import_file, $demo_id, $demo_data ) {
		...
		if ( ! is_array( $data ) && ( ! isset( $data['template'] ) || ! isset( $data['mods'] ) ) ) {
			return new WP_Error( 'mantrabrain_customizer_import_data_error', __( 'The customizer import file is not in a correct format. Please make sure to use the correct customizer import file.', 'mantrabrain-starter-sites' ) );
		}

		if ( ! empty( $demo_data['template'] ) && ! in_array( $data['template'], $demo_data['template'], true ) ) {
			return new WP_Error( 'mantrabrain_customizer_import_wrong_theme', __( 'The customizer import file is not suitable for current theme. You can only import customizer settings for the same theme or a child theme.', 'mantrabrain-starter-sites' ) );
		}

		// Import Images.
		if ( apply_filters( 'mantrabrain_customizer_import_images', true ) ) {
			$data['mods'] = self::import_customizer_images( $data['mods'] );
		}

		// Modify settings array.
		$data = apply_filters( 'mantrabrain_customizer_demo_import_settings', $data, $demo_data, $demo_id );
